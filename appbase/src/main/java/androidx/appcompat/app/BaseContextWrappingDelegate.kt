...
import com.digital.appktx.getUserLanguage

class BaseContextWrappingDelegate(private val superDelegate: AppCompatDelegate,
private val applyLocaleWrapping:Boolean) :
	AppCompatDelegate() {
	...
	override fun attachBaseContext2(context: Context) =
		wrap(superDelegate.attachBaseContext2(super.attachBaseContext2(context)),applyLocaleWrapping)
	...
	private fun wrap(context: Context, localeWrapEnable: Boolean = true): Context {
		return if (localeWrapEnable) changeAppLocale(getUserLanguage(context), context) {} else context
	}
}